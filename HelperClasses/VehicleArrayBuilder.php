...
class VehicleArrayBuilder
{
    public function __construct()
    {
        //using an export from edmunds in vehicles.JSON
        $json = $this->ApprovedVehicleJson();
    ...
    {
        $static = new static;
        return $static->getMakes();
    }

    public static function modelExists($make, $model)
    {
        return (new static)->hasModel($make, $model);
    }
    ...
    {
        return Collection::make($this->vehicles->makes)->map(function($make) {
            return $make->name;
        });
    }
     ...
     * @return bool
     */
    function hasModel($make, $model)
    {
        $make = strtolower(preg_replace("/[\s-]/", "", $make));
        $model = strtolower(preg_replace("/[\s-]/", "", $model));

        return Collection::make($this->vehicles->makes)
            ->contains(function($i, $data_make) use ($make, $model) {
                if (!$this->isMake($data_make, $make))
                    return false;

                return Collection::make($data_make->models)->contains(function($i, $data_model) use ($model) {
                    return $this->isModel($data_model, $model);
                });
            });

    }

    protected function isMake($data_make, $make)
    {
        $niceName = preg_replace("/[\s-]/", "", $data_make->niceName);
        if (strpos($niceName, $make) !== false)
            return true;
    }

    protected function isModel($data_model, $model)
    {
        $niceName = preg_replace("/[\s-]/", "", $data_model->niceName);
        if (strpos($niceName, $model) !== false
            && !in_array($model, ['one', 'van']))
            return true;
